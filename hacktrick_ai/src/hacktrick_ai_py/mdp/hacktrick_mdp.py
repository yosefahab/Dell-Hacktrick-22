...
from hacktrick_ai.src.hacktrick_ai_py.utils import pos_distance, read_layout_dict, classproperty
...
class HacktrickGridworld(object):
    ...
    def potential_function(self, state, mp, gamma=0.99):
        ...
        state_player = [player for player in state.players if player.position != (0, 0)]
        players_holding_solarlabs = [player for player in state_player if player.has_object() and player.get_object().name == 'solarlab']
        players_holding_containers = [player for player in state_player if player.has_object() and player.get_object().name == 'container']
        players_holding_laptops = [player for player in state_player if player.has_object() and player.get_object().name == Recipe.LAPTOP]
        players_holding_projectors = [player for player in state_player if player.has_object() and player.get_object().name == Recipe.PROJECTOR]
        players_holding_solar_cells = [player for player in state_player if player.has_object() and player.get_object().name == Recipe.SOLAR_CELL]
        players_holding_nothing = [player for player in state_player if not player.has_object()]
