...
import numpy as np
import pickle, time
from hacktrick_ai.src.hacktrick_ai_py.utils import manhattan_distance
from hacktrick_ai.src.hacktrick_ai_py.planning.search import Graph, NotConnectedError
from hacktrick_ai.src.hacktrick_ai_py.mdp.actions import Action, Direction
from hacktrick_ai.src.hacktrick_ai_py.mdp.hacktrick_mdp import HacktrickState, PlayerState, HacktrickGridworld, EVENT_TYPES
from hacktrick_ai.src.hacktrick_ai_py.data.planners import load_saved_action_manager, load_saved_motion_planner, PLANNERS_DIR
...
class MotionPlanner(object):
    ...
    arrive at goal positions and orientations in an HacktrickGridworld.
    Args:
        mdp (HacktrickGridworld): gridworld of interest
    ...
    def from_pickle_or_compute(mdp, counter_goals, custom_filename=None, force_compute=False, info=False):
        assert isinstance(mdp, HacktrickGridworld)
    ...
    def _graph_from_grid(self):
        """Creates a graph adjacency matrix from an Hacktrick MDP class."""
        state_decoder = {}
...
class JointMotionPlanner(object):
    ...
    arrive at goal positions and orientations in a HacktrickGridworld.
    Args:
        mdp (HacktrickGridworld): gridworld of interest
    """
    ...
    def _rollout_end_pos_and_or(self, joint_start_state, joint_action_plan):
        ...
        from hacktrick_ai.src.hacktrick_ai_py.mdp.hacktrick_env import HacktrickEnv
        dummy_state = HacktrickState.from_players_pos_and_or(joint_start_state, all_orders=self.mdp.start_all_orders)
        env = HacktrickEnv.from_mdp(self.mdp, horizon=200, info_level=int(self.debug)) # Plans should be shorter than 200 timesteps, or something is likely wrong
...
class MediumLevelActionManager(object):
    ...
    def get_medium_level_actions(self, state, player, waiting_substitute=False):
        ...
        Args:
            state (HacktrickState): current state
...
#     in an HacktrickGridworld using medium level actions (single motion goals) in the corresponding
#     A* search problem.
#     """
#
...
#
    ...
    #
    #         successor_state:   HacktrickState corresponding to state
    ...
    #             s_prime, _ = HacktrickEnv.execute_plan(self.mdp, start_state, joint_motion_action_plans, display=False)
    ...
    #
    #     Args:
    #         state (HacktrickState): starting state
    ...
    #         print("Start state")
    #         HacktrickEnv.print_state(self.mdp, start_state)
    #
    ...
    #         if not clean and debug:
    #             print(t)
    #             HacktrickEnv.print_state(self.mdp, goal_state)
    #
    #         if not clean and SAFE_RUN:
    #             s_prime, _ = HacktrickEnv.execute_plan(self.mdp, curr_state, joint_action_plan)
...
#     deliver a certain number of containers in an HacktrickGridworld
...
#
#         if not clean and debug:
#             env = HacktrickEnv.from_mdp(self.mdp)
#             env.state = state
#             print("\n" + "#"*35)
